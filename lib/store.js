...
class LocalStore {
  ...
  async updateBrand(input) {
    this.data.brand = {
      ...this.data.brand,
      ...pick(input, ["storeName", "localName", "hypermarketLabel", "headline", "subheadline", "phone", "validUntil", "deliveryText", "priceMode", "showDiscount", "logoDataUrl", "logoX", "logoY", "logoWidth"])
    };
    await this.save();
    return this.data.brand;
  }
  ...
  async saveBrandLogo(imageResult) {
    if (this.config.isVercel) {
      return {
        publicUrl: `data:${imageResult.mediaType};base64,${imageResult.buffer.toString("base64")}`,
        storagePath: "data-url-demo"
    ...
    const filename = `store-logo-${Date.now()}.${imageResult.extension}`;
    const filePath = path.join(this.generatedDir, filename);
    await fs.writeFile(filePath, imageResult.buffer);
    return {
      publicUrl: `/generated-products/${filename}`,
